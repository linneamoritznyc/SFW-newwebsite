...
## 19 Sep - donate block pulled

Stephanie McDaniel, on the review doc: "lets remove donate from this for now,
it gets complex with nation wide state laws and we don't want that to block the
completion and use of this wonderful trifold". Charitable solicitation is
registered state by state.

Removed the donate QR block from the Community panel foot, and dropped the
donate code from the printer package and its README. The code itself is still
generated by make-qr.py and the copy is still in the copy deck, so it goes back
in one block if the registration question is settled.

The green foot still opens at 5.97in, so the front-to-back harmony Linnea asked
for is unchanged. The address card stays bottom-aligned; there is now 1.42in of
empty green above it where the donate block was. Left empty rather than filled
with invented copy.

Still in the piece, not covered by Stephanie's note and not changed on my own
judgement: the DONATE pillar in the Study / Volunteer / Donate / Host row, and
